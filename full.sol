...
// MOONASS (MOASS) - Meme Utility Meets Defi
// 8% Tax (3% Burn // 5% back to Holders)
// OFFICIAL WEBSITE (https://www.moonass.com)
// Medium (https://moonass.medium.com)
// Twitter (https://twitter.com/slapmoonass)
// Telegram (https://t.me/slapMoonAss)
contract MOONASS is IBEP20, Ownable {
    ...
    uint256 private constant _tTotal = 350 * 10 ** 6 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = 'MOONASS';
    string private _symbol = 'MOASS';
    uint8 private _decimals = 9;

    event Send (address sender, address recipient, uint256 amount, string data);
    ...
    function send(address recipient, uint256 amount, string calldata data) public returns (bool) {
        _transfer(_msgSender(), recipient, amount);
        emit Send(_msgSender(), recipient, amount, data);
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        uint256 burnValues = _getBurnValues(tAmount);
        _reflectFee(rFee, tFee.sub(burnValues));
        _burn(sender, burnValues);
    ...
    function _whitelistTransfer(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
    ...
    function _getBurnValues(uint256 tAmount) private pure returns (uint256) {
